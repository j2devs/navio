...
const NotificationButton: React.FC = () => {
    ...
    const [notifications, setNotifications] = useState<Notification[]>([
        { id: 1, message: <><span className="font-semibold text-slate-900">John Doe</span> started following you.</>, time: '2 hours ago', read: false },
        { id: 2, message: <>Your trip to <span className="font-semibold text-slate-900">Paris</span> is in 3 days.</>, time: '1 day ago', read: false },
        { id: 3, message: <>Welcome to <span className="font-semibold text-slate-900">Navio</span>!</>, time: '3 days ago', read: true },
    ]);
    ...
    return (
        <div className="relative" ref={menuRef}>
            ...
            >
                <svg className={`w-6 h-6 ${isOpen ? 'text-white' : 'text-gray-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                ...
                {hasUnread && <span className="absolute top-0 right-0 w-3 h-3 bg-red-500 rounded-full border-2 border-white animate-pulse" />}
            </button>
            <div className={`absolute right-0 mt-2 w-80 bg-white rounded-2xl shadow-2xl p-4 transition-all duration-300 ease-in-out ${isOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}`}>
